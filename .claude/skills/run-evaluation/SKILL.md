...
Each file defines a `script` (the model server Python file) and `args` (model-specific flags). Example (`configs/model_servers/openvla/openvla.yaml`):
```yaml
...
docker:
  image: ghcr.io/worv-ai/vla-evaluation-harness/libero:latest
output_dir: "./results"
benchmarks:
...
5. **Docker image** — Benchmark images are large (4–10 GB). `vla-eval run` prompts to pull if missing. Pre-pull with: `docker pull ghcr.io/worv-ai/vla-evaluation-harness/<benchmark>:latest`
...
# Terminal 1:
vla-eval serve -c configs/model_servers/openvla/openvla.yaml
